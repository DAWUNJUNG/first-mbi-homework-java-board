...
public class DojeDao {
	...
	public List<Users> getUserList(HashMap<String, Object> hashMap) {
		long stasrtTime = System.currentTimeMillis();
		SqlSession session = null;
		List<Users> result = null;
		try {
			session = mSqlSessionFactory.openSession(true);
			result = session.selectList("mysql.getUserList", hashMap);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
		} finally {
		...
		logger.debug("getUserList : " + responseTime + "ms");
		return result;
	}
	...
	public List<Users> findUserList(HashMap<String, Object> hashMap) {
		...
		try {
			session = mSqlSessionFactory.openSession(true);
			result = session.selectList("mysql.getUserList");
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
		} finally {
